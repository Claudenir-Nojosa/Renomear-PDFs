...
def renomear_arquivo_com_razao_social(caminho_arquivo, competencia, razao_social):
    diretorio, nome_arquivo = os.path.split(caminho_arquivo)
    _, extensao = os.path.splitext(nome_arquivo)
    competencia_limpa = re.sub(r'[\\/*?:"<>|]', '', competencia)
    razao_social_limpa = re.sub(r'[\\/*?:"<>|]', '', razao_social)
    novo_nome_arquivo = f"CERT ISS {competencia_limpa} {razao_social_limpa}{extensao}"
...
def alterar_pdf_certificados():
    for nome_arquivo in os.listdir(diretorio):
      ...
      if nome_arquivo.endswith(".pdf"):
          ...
          competencia = extrair_texto_entre_palavras(pdf_texto, 'Competência:', 'Número do Protocolo')
